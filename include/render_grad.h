...
// need to define this, or else picture will not be drawn correctly:
#ifndef RENDER_GLOBAL_ILLUMINATION_H
#define RENDER_GLOBAL_ILLUMINATION_H

#define ERPT_MUTATIONS 1
...
typedef struct render_t
{
  ...
  float *grad_x;  // gradient x direction
  float *grad_y;  // gradient y direction
...
static inline render_t *render_init()
{
  ...
  r->grad_x = (float *)malloc(sizeof(float)*rt.width*rt.height*3);
  r->grad_y = (float *)malloc(sizeof(float)*rt.width*rt.height*3);
  ...
  memset(r->grad_x, 0, sizeof(float)*rt.cam->width*rt.cam->height*3);
  memset(r->grad_y, 0, sizeof(float)*rt.cam->width*rt.cam->height*3);
...
static inline void render_clear()
{
  ...
  memset(rt.render->grad_x, 0, sizeof(float)*rt.cam->width*rt.cam->height*3);
  memset(rt.render->grad_y, 0, sizeof(float)*rt.cam->width*rt.cam->height*3);
...
static inline void render_print_info(FILE *fd)
{
  ...
  fprintf(fd, "render   : global illumination, writing gradient images\n");
...
static inline void render_screenshot(const char *tempname)
{
  ...
  {
    ((__m128*)(rt.render->pixel))[k] = _mm_setzero_ps();
    for(int i=0;i<3;i++) rt.render->pixel[4*k + i + 1] = rt.render->grad_x[3*k + i];
    ((__m128*)(rt.render->pixel))[k] = _mm_mul_ps(((__m128*)(rt.render->pixel))[k], inv_o);
  }
  snprintf(gradname, 512, "%s_grad_x", tempname);
  screenshot_write(gradname, rt.render->pixel, 4);
  for(int k=0;k<rt.width*rt.height;k++)
  {
    ((__m128*)(rt.render->pixel))[k] = _mm_setzero_ps();
    for(int i=0;i<3;i++) rt.render->pixel[4*k + i + 1] = rt.render->grad_y[3*k + i];
    ((__m128*)(rt.render->pixel))[k] = _mm_mul_ps(((__m128*)(rt.render->pixel))[k], inv_o);
  }
  snprintf(gradname, 512, "%s_grad_y", tempname);
  screenshot_write(gradname, rt.render->pixel, 4);
}
#endif
