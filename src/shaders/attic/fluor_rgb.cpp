...
void get_rgb2(const float theta_in, const float theta_out, const float phi_rel, multispecBRDF *s, float *rgb, int adjoint)
{
  // FIXME:
  adjoint = 1;
  ...
  for(int i=0;i<M;i++)
  {
    ...
    for(int k=0;k<N;k++)
    {
      ...
      const float f = 300.0f*300.0f*rd*ENERGY_SCALE/((float)M*N);
...
extern float sample(const float *omega_in, rayhit_t *hit, float *omega_out, const float x1, const float x2, const float rr, void *data)
{
  ...
  float rgb[3];
  get_rgb2(theta_in, theta_out, phi_rel, f->s, rgb, hit->adjoint);
#else
  float *rgb = (float *)data;
#endif
  return M_PI*spectrum_rgb_to_p(hit->lambda, rgb);
}
...
extern float brdf(float *omega_in, rayhit_t *hit, float *omega_out, void *data)
{
  ...
  if(phi_rel > M_PI) phi_rel = 2.0f*M_PI - phi_rel;
  float rgb[3];
  get_rgb2(theta_in, theta_out, phi_rel, f->s, rgb, 1);
#else
  float *rgb = (float *)data;
#endif
  return spectrum_rgb_to_p(hit->lambda, rgb);
}
...
extern int init(FILE *f, void **data)
{
  ...
  *data = rgb;
  ...
  get_rgb2(.4, .4, M_PI, s, rgb, 1);

  printf("[fluor_rgb] rgb: %f %f %f\n", rgb[0], rgb[1], rgb[2]);
  for(int k=0;k<3;k++) rgb[k] = fminf(1.0, fmaxf(rgb[k], 0.0f));
