...
typedef struct
{
  qbvh_float4_t aabb[6];
  int child[4];  // child index or, if leaf -(prim<<5|num_prims) index
  int parent;
  int axis0;
  int axis00;
  int axis01;
...
typedef struct accel_t
{
  ...
  unsigned int *shadow_cache;
  unsigned int shadow_cache_last;
...
const float *accel_aabb(const accel_t *a)
{
  return a->aabb;
}
...
void checktree(accel_t *b, qbvh_node_t *node, qbvh_stats_t *stats, const int depth)
{
  ...
  float child_box[4];
  for(int c=0;c<4;c++)
    child_box[c] = (node->aabb[5].f[c] - node->aabb[2].f[c])*(node->aabb[4].f[c] - node->aabb[1].f[c]) +
                   (node->aabb[3].f[c] - node->aabb[0].f[c])*(node->aabb[4].f[c] - node->aabb[1].f[c]) +
                   (node->aabb[5].f[c] - node->aabb[2].f[c])*(node->aabb[3].f[c] - node->aabb[0].f[c]);
  stats->num_nodes++;
  ...
  assert(node - b->tree == 0 || (node->parent >= 0 && node->parent < b->num_nodes));
  for(int c=0;c<4;c++)
  {
    int tris_before = stats->num_tris;
    // leaf?
    if(node->child[c] & 0x80000000UL)
    {
      stats->num_leaves++;
      stats->sum_depth += depth;
      if(depth > stats->max_depth) stats->max_depth = depth;
      const int num_prims = node->child[c] & ~-(1<<5);
      const int prims = (node->child[c] ^ 0x80000000UL) >> 5;
      if(num_prims > 0)
      {
        stats->num_tris += num_prims;
        if(prims + num_prims > (int)b->prims->num_prims) fprintf(stderr, "ERR: prims index out of range!\n");
      }
...
accel_t* accel_init(prims_t *prims)
{
  ...
  //if(posix_memalign((void**)&(b->tree), 128, b->node_bufsize * sizeof(qbvh_node_t)/NUM_TRIS_PER_LEAF))
  if(!b->tree)
...
#else
    prims_get_bounds(b->prims, b->prims->primid[k], d, &min, &max);
#endif
...
void sort_tris_no_both(accel_t *b, const int axis, const float split, const int left, const int right, unsigned int *pivot, float *aabblo, float *aabbro)
{
  ...
  {
#ifndef BVH_SSE_SAH
    prims_get_bounds(b->prims, b->prims->primid[i], axis, &pmin, &pmax);
#else
    ...
    if(0.5f*(pmin + pmax) >= split)
    {
      ...
      prims_get_bounds(b->prims, b->prims->primid[i], p, &pmin, &pmax);
      if(aabbr[p] > pmin) aabbr[p] = pmin;
      if(aabbr[p+3] < pmax) aabbr[p+3] = pmax;
      prims_get_bounds(b->prims, b->prims->primid[i], q, &pmin, &pmax);
...
void build_help(accel_t *b, qbvh_node_t *node, const int left, const int right, float *paabb, const int depth, qbvh_node_t *parent, const int child)
{
  ...
  if(best_score < 0.0f && (right - left) < 32)
  {
    ...
    if(parent)
    {
      ...
      parent->child[child] = 0x80000000 | (left<<5) | ((right - left) & ~-(1<<5));
      return;
    }
  }
#endif
...
do_retry:
...
#endif
    {
      ...
      for(int p=0;p<4;p++)
      {
        for(unsigned int k=part[p];k<part[p+1];k++)
        {
          ...
          for(int i=0;i<3;i++)
          {
            prims_get_bounds(b->prims, b->prims->primid[k], i, &min, &max);
  ...
  node->axis00 = axis00;
  node->axis01 = axis01;
  for(int k=0;k<6;k++) for(int p=0;p<4;p++) node->aabb[k].f[p] = aabb[p][k];
...
#pragma omp parallel for if(right-left > 20000)
  ...
  for (int p=0;p<4;p++)
  {
    if (depth == MAX_TREE_DEPTH || part[p+1] - part[p] <= NUM_TRIS_PER_LEAF || b->num_nodes >= b->node_bufsize - 1)
      node->child[p] = 0x80000000 | (part[p]<<5) | ((part[p+1] - part[p]) & ~-(1<<5));
    else
    {
#ifdef BUILD_IN_PARALLEL
...
#if 0
  qbvh_stats_t stats;
  memset(&stats, 0, sizeof(qbvh_stats_t)); stats.SA = 0.0f;
  checktree(b, b->tree, &stats, 0);
  printf("[accel] num nodes created: %d\n", b->num_nodes);
  printf("[accel] created qbvh with %d nodes, %d leaves (avg %.3f/%.3f depth/num tris, %d empty), %d tris, max depth is %d.\n",
...
static void aabb_intersect(const qbvh_float4_t *aabb, const __m128 *pos4, const __m128 *invdir4, const int *near, const int *far, qbvh_float4_t *tmin, qbvh_float4_t *tmax)
{
  // padding 4ulp proposed by thiago ize (http://www.solidangle.com/research/jcgt2013_robust_BVH.pdf)
  // did not result in better images on highly tessellated faces (maybe origin was too close?)
  // const __m128 pad = _mm_set1_ps(1.00000024f);
  for(int k=0;k<3;k++)
  {
    __m128 t[2];
#if 0//def __FMA__
    // FIXME: this code is currently broken (and wasn't faster on cpus that supported fma)
    // FIXME: it needs help in the calling ray intersection code, to pre-mul invdir to pos4.
    t[0] = _mm_fmsub_ps(aabb[k+3*near[k]].m, pos4[k], invdir4[k]);
    t[1] = _mm_fmsub_ps(aabb[k+3*far [k]].m, pos4[k], invdir4[k]);
#else
    t[0] = _mm_mul_ps(_mm_sub_ps(aabb[k+3*near[k]].m, pos4[k]), invdir4[k]);
    t[1] = _mm_mul_ps(_mm_sub_ps(aabb[k+3*far [k]].m, pos4[k]), invdir4[k]);
#endif
    tmin->m = _mm_max_ps(tmin->m, t[0]);
    tmax->m = _mm_min_ps(tmax->m, t[1]);
  }
  // tmax->m = _mm_mul_ps(tmax->m, pad);
}
...
void accel_intersect(const accel_t *b, const ray_t *ray, hit_t *hit)
{
  ...
  for(int k=0;k<3;k++)
  {
    near[k] = (*(unsigned int*)(&(ray->dir[k])))>>31;//(ray->dir[k] > 0.0f) & 1; for strict aliasing.
    far[k] = 1 ^ near[k];
  }
  ...
  while(1)
  {
    ...
    //((accel_t *)b)->boxtests++;
    aabb_intersect(node->aabb, pos4, invdir4, near, far, &tmin4, &tmax4);
    ...
    while(current & 0x80000000)
    {
      ...
      unsigned int tmp_index = (current ^ 0x80000000) >> 5;
      const unsigned int num = current & ~-(1<<5);
